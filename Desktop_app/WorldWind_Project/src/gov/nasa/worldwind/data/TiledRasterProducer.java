...
public abstract class TiledRasterProducer extends AbstractDataStoreProducer
{
    ...
    public TiledRasterProducer(Sector bbox)
    {
    ...
    protected Tile[] createSubTiles(Tile tile, Level nextLevel)
    {
        ...
        Sector sector1 = new Sector(p0, p1, t0, t1);
        if(bbox.intersects(sector1)){
        	subTiles.add(new Tile(sector1, nextLevel, 2 * row, 2 * col));
        }
        
        Sector sector2 = new Sector(p0, p1, t1, t2);
        if(bbox.intersects(sector2)){
        	subTiles.add(new Tile(sector2, nextLevel, 2 * row, 2 * col + 1));
        }
        
        Sector sector3 = new Sector(p1, p2, t1, t2);
        if(bbox.intersects(sector3)){
        	subTiles.add(new Tile(sector3, nextLevel, 2 * row + 1, 2 * col + 1));
        }
        
        Sector sector4 = new Sector(p1, p2, t0, t1);
        if(bbox.intersects(sector4)){
        	subTiles.add(new Tile(sector4, nextLevel,  2 * row + 1, 2 * col));
        }
        ...
        return subTiles.toArray(new Tile[subTiles.size()]);
    }
    ...
    throws IOException{
        Sector sector = levelSet.getSector().intersection(bbox);
        ...
        if(null!=sector){
        	...
        	throw new IOException("No intersection between bounding box and tile data");
        }
    }
