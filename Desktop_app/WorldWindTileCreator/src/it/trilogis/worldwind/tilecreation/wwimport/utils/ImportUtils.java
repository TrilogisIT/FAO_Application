...
public class ImportUtils {
	...
	public static void createRasterServerConfigDoc(File directory,DataStoreProducer producer) {
		...
		if (WWUtil.isEmpty(o)) productionParams.setValue(AVKey.DISPLAY_NAME, productionParams.getValue(AVKey.DATASET_NAME));
		...
		if (productionParams.hasKey(AVKey.SECTOR)) {
			...
			if (null != o && o instanceof Sector) extent = (Sector) o;
		}

		if (null != extent) WWXML.appendSector(root, "Sector", extent);
		else {
	...
	public static void appendSource(Element sources, CachedDataRaster raster) throws WWRuntimeException {
		...
		if (null == sector && params.hasKey(AVKey.SECTOR)) {
			...
			if (o instanceof Sector) sector = (Sector) o;
		}

		if (null != sector) WWXML.appendSector(source, "Sector", sector);
	}
	
	public static DataStoreProducer createDataStoreProducerFromFiles(File[] files,Sector bbox,boolean genPKM) throws IllegalArgumentException {
		...
		if (AVKey.IMAGE.equals(commonPixelFormat)) {
			if(genPKM){
				...
				return new TransparentTiledImageProducer(bbox);
			}
