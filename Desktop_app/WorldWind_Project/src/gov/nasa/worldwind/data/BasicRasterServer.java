    ...
    public DataRaster composeRaster(AVList reqParams) throws IllegalArgumentException, WWRuntimeException
    {
        ...
        try
        {
            ...
            else if (AVKey.IMAGE.equals(this.getStringValue(AVKey.PIXEL_FORMAT)))
            {
                ...
                //TODO get from params the boolean
                boolean replace = false;
                if (null!=reqParams && null!=reqParams.getValue("TRILOGIS")){
                	replace = (Boolean) reqParams.getValue("TRILOGIS");
        		}
                
                reqRaster = new BufferedImageRaster(reqWidth, reqHeight, Transparency.TRANSLUCENT, reqSector,replace);
            }
            else
            {
