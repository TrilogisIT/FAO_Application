...
/**
 * @author nmeneghini
 * @version $Id: TiledImageProducer.java 1 2014-05-01 15:22:47Z nmeneghini $
 */
public class TransparentTiledImageProducer extends TiledRasterProducer {
	...
	public TransparentTiledImageProducer(MemoryCache cache, int writeThreadPoolSize) {
		super(cache, writeThreadPoolSize);
	}

	public TransparentTiledImageProducer(Sector bbox) {
		super(bbox);
	}
	...
	protected DataRaster createDataRaster(int width, int height, Sector sector, AVList params) {
		...
		BufferedImageRaster raster = new BufferedImageRaster(width, height, transparency, sector,replaceWhitePixels);
		return raster;
	}
...
//				 new ECT1DDSRasterWriter(),new ImageIORasterWriter(false), new DDSRasterWriter() };
				new ImageIORasterWriter(false),new DDSRasterWriter() };
//				new PKMRasterWriter() };
	}
